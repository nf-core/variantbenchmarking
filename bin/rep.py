...
# Vendored layout for the nf-core local module: rep.py lives in the pipeline
# bin/ (added to PATH by Nextflow) and its assets sit alongside it in
# bin/ga4gh_report_assets/. Resolve relative to this file's real location so it
# works whether invoked by PATH lookup or absolute path. GA4GH_REPORT_ROOT can
# override the asset root if the layout ever changes.
_SELFDIR = os.path.dirname(os.path.realpath(__file__))
_ASSETS = os.environ.get("GA4GH_REPORT_ROOT",
                         os.path.join(_SELFDIR, "ga4gh_report_assets"))
TEMPLATEDIR = os.path.abspath(os.path.join(_ASSETS, "html"))
LIBDIR = os.path.abspath(_ASSETS)
