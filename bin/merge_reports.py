...
def parse_args(args=None):
    ...
    Epilog = "Example usage: python merge_reports.py file1 file2 file3 -o merged_table.csv -b truvari/svbenchmark/wittyer/happy/sompy/rtgtools/concordance/intersect/aardvark -v snv/indel/structural/copynumber -a germline/somatic "

    parser = argparse.ArgumentParser(description=Description, epilog=Epilog)
    parser.add_argument("inputs", nargs="+", help="List of files to merge")
    parser.add_argument("--output", "-o", required=True, help="Output file")
    parser.add_argument("--bench", "-b", required=True, help="truvari/svbenchmark/wittyer/happy/sompy/rtgtools/concordance/intersect/aardvark")
...
def get_rtgtools_results(file_paths):
    ...
    for file in file_paths:
        ...
        if any("0 total baseline variants, no summary statistics available" in line for line in lines) or len(lines) < 3:
            ...
            try:
                df_redesigned = df[['Tool', 'File', 'Caller', 'Threshold','True-pos-baseline','True-pos-call',
									'False-pos','False-neg','Precision','Sensitivity','F-measure']].copy()
                df_redesigned.columns = ['Tool','File', 'Caller', 'Threshold','TP_base','TP_comp',
...
def get_happy_results(file_paths):
    ...
    for file in file_paths:
        ...
        df_redesigned = df[['Tool', 'File', 'Caller', 'Type','Filter','TRUTH.TOTAL','TRUTH.TP',
							'TRUTH.FN','QUERY.TOTAL','QUERY.FP','QUERY.UNK','FP.gt','FP.al',
							...
							'QUERY.TOTAL.het_hom_ratio']].copy()
        df_redesigned.columns = ['Tool', 'File', 'Caller', 'Type','Filter','TP_base','TP_comp',
								'FN','TP_Total','FP','UNK','FP_gt','FP_al','Recall','Precision','Frac_NA','F1',
								'TRUTH_TiTv_ratio','QUERY_TiTv_ratio','TRUTH_het_hom_ratio',
								'QUERY_het_hom_ratio']
...
def get_concordance_results(file_paths):
    ...
    for file in file_paths:
        ...
        df[int_columns] = df[int_columns].fillna(0).astype(int)
        df[float_columns] = df[float_columns].fillna(0).astype(float)
        df_redesigned = df[['Tool','File','type', 'TP', 'FP', 'FN','RECALL','PRECISION','F1']].copy()
...
def get_sompy_results(file_paths, vartype):
    ...
    for file in file_paths:
        ...
        df['F1'] = 2 * (df["precision"] * df["recall"]) / (df["precision"] + df["recall"])
        df['F1'] = df['F1'].fillna(0)
        df_redesigned = df[['Tool','File', 'Caller', 'F1', 'type','total.truth','tp','fn',
							'total.query','fp','unk','recall','precision','recall_lower',
							'recall_upper','recall2','precision_lower','precision_upper',
							'na','ambiguous','fp.region.size','fp.rate']].copy()
        df_redesigned.columns = ['Tool', 'File', 'Caller', 'F1', 'Type','TP_base','TP_comp','FN',
								'TP_Total','FP','UNK','Recall','Precision','recall_lower',
								'recall_upper','recall2','precision_lower','precision_upper',
								'na','ambiguous','fp.region.size','fp.rate']

        int_columns = ['TP_base', 'FN', 'TP_comp', 'FP', 'UNK']
        float_columns = ['Recall','Precision','recall_lower','recall_upper','recall2',
						'precision_lower','precision_upper','na','ambiguous','fp.region.size','F1']
        df_redesigned[int_columns] = df_redesigned[int_columns].fillna(0).astype(int)
        df_redesigned[float_columns] = df_redesigned[float_columns].fillna(0).astype(float)
...
def get_aardvark_results(file_paths):
    merged_df = pd.DataFrame()
...
def main(args=None):
    ...
    elif args.bench == "aardvark":
        summ_table = get_aardvark_results(args.inputs)

    else:
        raise ValueError('only results from concordance, intersect, truvari, svbenchmark, wittyer, rtgtools, happy, sompy or aardvark tools can be merged')
