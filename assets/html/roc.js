...
(function (PLOT) {
  ...
  var DASHTYPES = [null, "4,4", "2,2", "1,5", "0.9", "5,5,1,5"];
  ...
  function highlightSingle(svg, plot_g, d) {
    ...
    plot_g.selectAll(id).style("stroke", "black");
    svg.selectAll(".legend_" + d.method.replace(/[^A-Z0-9_]/gi, "_")).style("font-weight", "bold");
    svg.selectAll(".cmlegend_" + d.comparisonmethod.replace(/[^A-Z0-9_]/gi, "_")).style("font-weight", "bold");
    plot_g.selectAll(id.replace(/\.dot/g, ".roc_ALL")).style("stroke-width", "2px").style("opacity", 0.5);
    plot_g.selectAll(id.replace(/\.dot/g, ".roc_PASS")).style("stroke-width", "4px").style("opacity", 1.0);
    plot_g.selectAll(id.replace(/\.dot/g, ".pf")).style("stroke", "black").style("opacity", 0.8);
  }

  /** highlight single dataset */
  function unhighlightSingle(svg, plot_g, d) {
    var id = ".dot_" + makeDataID(d, ["type", "subtype", "subset", "method", "comparisonmethod"]);
    plot_g.selectAll(id).style("stroke", function (d) {
      if (d.filter != "ALL") {
        return d.FILL;
      } else {
        return "black";
    ...
    svg.selectAll(".legend_" + d.method.replace(/[^A-Z0-9_]/gi, "_")).style("font-weight", "normal");
    svg.selectAll(".cmlegend_" + d.comparisonmethod.replace(/[^A-Z0-9_]/gi, "_")).style("font-weight", "normal");
    plot_g.selectAll(id.replace(/\.dot/g, ".roc_ALL")).style("stroke-width", "2px").style("opacity", 0.0);
    plot_g.selectAll(id.replace(/\.dot/g, ".roc_PASS")).style("stroke-width", "2px").style("opacity", 0.5);
    plot_g.selectAll(id.replace(/\.dot/g, ".pf")).style("stroke", "grey").style("opacity", 0.8);
  }
  ...
  PLOT.roc = function (target, data) {
    ...
    var xAxis = d3.svg.axis().scale(x).tickFormat(d3.format(".2%")).ticks(6).orient("bottom");

    var yAxis = d3.svg.axis().scale(y).ticks(4).tickFormat(d3.format(".2%")).orient("left");
    ...
    var zoom = d3.behavior.zoom().x(x).y(y).scaleExtent([0.01, 5]).on("zoom", zoomed);

    var plot_g = svg.append("g").attr("clip-path", "url(#clip)");
    plot_g
      .append("rect")
      .attr("x", 0)
      .attr("y", 0)
      ...
      .append("svg:title")
      .text("Shift-click to zoom, ctrl-click to reset.");
      ...
      plot_g
        ...
        .attr("class", function (d) {
          return (
            "pf" +
            " pf_ " +
            d.method.replace(/[^A-Z0-9_]/gi, "_") +
            " pf_" +
            makeDataID(d, ["type", "subtype", "subset", "method", "comparisonmethod"])
          );
        })
        .attr("x1", function (d) {
          return d.x1;
      ...
      plot_g
        ...
        .data(
          data.filter(function (d) {
            return d.filter == "PASS" || d.filter == "ALL";
          }),
        )
        .enter()
        .append("path")
    ...
    cmlegend
      ...
      .attr("transform", function (d) {
        return "translate(" + (legend_start - 9) + "," + 9 + ")scale(1.5)";
      })
      .style("stroke", "black");
    cmlegend
      .append("text")
